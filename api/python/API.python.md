...
| <code><a href="#@cnab/core.CnabFile.parseToJson">parse_to_json</a></code> | Parse file content and return the result as a **single JSON string**: an array of objects shaped exactly like `ParsedLine` — `[{"recordKey":…,"tipo":…,"segment":…,"fields":{…}}]`, camelCase keys, the same values `parse` produces, in the same order. |
...
##### `parse_to_json` <a name="parse_to_json" id="@cnab/core.CnabFile.parseToJson"></a>
